...
def transcribe(audio_url, auto_chapters):
    ...
    transcript_response = requests.post(
        transcript_endpoint, json=transcript_request, headers=headers_assemblyai)
...
def poll(transcript_id):
    ...
    polling_response = requests.get(
        polling_endpoint, headers=headers_assemblyai)
...
def save_transcript(episode_id):
    audio_url, thumbnail, podcast_title, episode_title = get_episode_audio_url(
        episode_id)
